          ...
          <slot :name="header.prop" :row="scope.row" :$index="scope.$index">
            <div v-if="scope.row && scope.row.__edit">
              ...
              <div
                ...
                class="error-message"
              >
                {{ scope.row.__validateMsgs[header.prop] }}
              </div>
            </div>
            <span v-else>{{
              scope.row
                ? header.formatter
                  ? header.formatter(
                      extractProp(scope.row, header.prop),
                      scope.row
                    )
                  : extractProp(scope.row, header.prop) !== undefined
                  ? extractProp(scope.row, header.prop)
...
export default {
  ...
  methods: {
    extractProp(row, propStr) {
      if (!propStr) {
        return undefined;
      }
      const props = propStr.split(".");
      let currentVal = row[props[0]];
      props.forEach((p, index) => {
        if (index > 0 && currentVal !== undefined) {
